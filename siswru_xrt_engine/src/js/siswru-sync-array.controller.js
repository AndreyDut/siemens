 ...
 */
app.controller( 'siswruSyncArrayController', [
	'$rootScope',
    '$scope',
    '$element',
    'uwPropertyService',
    'uwSupportService',
    'localeService',
    function($rootScope, $scope, $element, uwPropertySvc, uwSupportSvc, localeSvc ) {
        if( !$scope.prop ) {
            return;
        }
        ...
        $scope.dummyProp.updateArray = function() {
            ...
            if( self.isValidArrayValue( $scope.dummyProp ) && !$scope.prop.error && self.validToAdd( $scope.prop ) ) {
                ...
                if( _.isArray( $scope.dummyProp.dbValue ) ) {
                    ...
                    eventBus.publish('AddItem', {prop: $scope.prop.propertyName, length: $scope.prop.dbValue.length});
       ...
       let subsAdd = eventBus.subscribe('AddItem', async function(args) {
            if ($scope.prop.propertyName != args.prop 
                && $scope.prop.syncArray 
                && $scope.prop.dbValue.length != args.length) {
                    switch($scope.prop.type) {
                        case "INTEGERARRAY":
                            $scope.prop.dbValue.push( 0 );
                            break;
                        case "STRINGARRAY":
                            $scope.prop.dbValue.push( 'ENTER VALUE' );
                            break;
                        case "DATEARRAY":
                            $scope.prop.dbValue.push( new Date() );
                            break;
                    }
               uwPropertySvc.updateViewModelProperty( $scope.prop );    
            }
        ...
        $scope.moveUp = function( $index ) {
            if( $index > 0 ) {
                moveUp($index);
                eventBus.publish('moveUp', {index: $index, prop: $scope.prop.propertyName}, true);
            }
        };
        let subsMoveUp = eventBus.subscribe('moveUp', function (args) {
			if ($scope.prop.propertyName != args.prop && $scope.prop.syncArray ) {
                if( args.index > 0 ) {
                    moveUp(args.index);
                }
			}
        ...
        $scope.moveDown = function( $index ) {
            if( $index <= $scope.prop.displayValsModel.length - 1 ) {
                moveDown($index)
            //fire event to other lists
            $rootScope.$broadcast('moveDown', {index: $index, prop: $scope.prop.propertyName});
            //eventBus.publish('moveDown', {index: $index, prop: $scope.prop.propertyName}, true);
            }
			
        };
        $scope.$on('moveDown', function(event, args){
            if ($scope.prop.propertyName != args.prop && $scope.prop.syncArray ) {
                if( args.index <= $scope.prop.displayValsModel.length - 1 ) {
                    moveDown(args.index)
                }
            }
        });
		// eventBus.subscribe('moveDown', function (args) {
		// 	if ($scope.prop.propertyName != args.prop && $scope.prop.syncArray ) {
        //         if( args.index <= $scope.prop.displayValsModel.length - 1 ) {
        //             moveDown(args.index)
        //         }
        ...
        $scope.remove = function( $index ) {
            remove($index);
            eventBus.publish('remove', {index: $index, prop: $scope.prop.propertyName}, true);
            //$rootScope.$broadcast('remove', {index: $index, prop: $scope.prop.propertyName});
        };
        let subsRemove = eventBus.subscribe('remove', function (args) {
            if ($scope.prop.propertyName != args.prop && $scope.prop.syncArray ) {
                remove(args.index);
            }
        });
        $scope.$on('$destroy', () => {
            eventBus.unsubscribe(subsRemove);
            eventBus.unsubscribe(subsMoveUp);
            eventBus.unsubscribe(subsAdd);
        });
    }
] );
