...
export async function loadColumns(uwDataProvider, relationInfo){
    ...
    awColumnInfos.push(createColumn('object_string',"Наименование", 300, 100, false ))
    if(relationInfo.relationTypeName === 'CMHasImpactedItem'){
        awColumnInfos.push(createColumn('spd5_Zadel',"Указание о задаеле", 150, 50, true ))
    }
    if(relationInfo.actionTypeName === "Ввести взамен"){
        awColumnInfos.push(createColumn('spd5_dm',"Изменение массы", 150, 50, false )) 
    }
